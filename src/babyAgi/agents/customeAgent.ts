...
Execute the task in the CONTEXT of the accomplishing following objective: {objective}
You can assume the following tasks have been completed, and you should NOT do them again: {context}
...
class CustomAgent {
  ...
  async execute(input: string, objective: string, context: string[]): Promise<string> {
    ...
    const result = await this.executor.call({ input, objective, context: context.join("\n") });
